...
root_folder = "/home/roy/MEGAsync/outputs/NMR/experiments/BMRB-500-100mM"
...
project_path = joinpath(root_folder, project_name)
...
[hcat(cbr[collect(1:length(inds))]...); collect(1:length(inds))'] # all groups. last row is index.

# next, do for all proteogenic compounds.
# hypothesis, for the large amplitude, mag eq groups, they have equal split in cbr.
# - try merging the groups that sum to the same -1 to the groups.
