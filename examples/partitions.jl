...
push!(project_names, "glucose-500-2mM")
push!(project_names, "glucose-500-0.5mM")
#push!(project_names, "glucose-500-100mM")
push!(project_names, "glucose-nrc-2018")
push!(project_names, "Nam-Jan2022")
#push!(project_names, "Nam-Apr2021")
push!(project_names, "glucose-700")
...
function getΔcinfo!(part_dic, mixture_params_dic, project_name; verbose_flag = true)
    ...
    for i = 1:length(A.Ωs)
        ...
        #means_ps = collect( Statistics.mean(ps[k]) for k = 1:length(ps) )
        means_ps = A.Δc_avg[i]

        ds = collect( collect(ps[k][l] - means_ps[k] for l = 1:length(ps[k])) for k = 1:length(ps))
        ns = collect( norm.(ds[k]) for k = 1:length(ds) )
        max_ns = collect( maximum(ns[k]) for k = 1:length(ns))

        part_dic["$(project_name)_spin_group_$(i)"] = Dict("max_ns" => max_ns, "means_ps" => means_ps)

        if verbose_flag
            println("maximum l-2 discrepancy between mean and each Δc for $(project_name), spin group $(i):")

            max_ns_rounded = collect( round.(max_ns[k], sigdigits = 2) for k = 1:length(max_ns))
            display(max_ns_rounded)
            println()

            means_ps_rounded = collect( round.(means_ps[k], sigdigits = 2) for k = 1:length(means_ps))
            println("mean Δc for $(project_name), spin group $(i):")
            display(means_ps_rounded)
...
for project_name in project_names
    mixture_params_dic["$(project_name)"] = getsimulation(projects_dir, project_name, molecule_names, tol_coherence, α_relative_threshold, Δc_partition_radius;
...
means_ps_rounded = collect( round.(means_ps[k], digits = 2) for k = 1:length(means_ps))
